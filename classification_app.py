...
Displays results from 2 ML models, 2 DL models, and 1 Transformer model
"""
...
def display_model_metrics(metrics, model_name, set_name="Test"):
    ...
    with col2:
        st.metric("Exact Match", f"{metrics.get('exact_match', 0):.4f}")
...
def compare_models(all_metrics):
    ...
    # Accuracy comparison
    axes[0, 0].bar(models, accuracies, color=['#FF6B35', '#F7931E', '#667eea', '#764ba2', '#4CAF50'])
    ...
    # F1-Score comparison
    axes[0, 1].bar(models, f1_scores, color=['#FF6B35', '#F7931E', '#667eea', '#764ba2', '#4CAF50'])
    ...
    axes[1, 0].bar(models, precisions, color=['#FF6B35', '#F7931E', '#667eea', '#764ba2', '#4CAF50'])
    ...
    # Recall comparison
    axes[1, 1].bar(models, recalls, color=['#FF6B35', '#F7931E', '#667eea', '#764ba2', '#4CAF50'])
...
def main():
    ...
    st.markdown('<p class="sub-header">2 ML Models | 2 DL Models | 1 Transformer Model</p>', unsafe_allow_html=True)
    ...
    # Model names
    ml_models = ["Random Forest", "SVM"]
    dl_models = ["LSTM", "CNN"]
    transformer_models = ["BERT"]
    all_models = ml_models + dl_models + transformer_models
        ...
        if model_name in all_metrics and 'test' in all_metrics[model_name]:
            ...
            if test_metrics:
                comparison_data.append({
                    'Model': model_name,
                    'Type': 'ML' if model_name in ml_models else ('DL' if model_name in dl_models else 'Transformer'),
                    'Accuracy': test_metrics.get('accuracy', 0),
                    'Precision': test_metrics.get('precision', 0),
                    'Recall': test_metrics.get('recall', 0),
                    'F1-Score': test_metrics.get('f1_score', 0),
                    'AUC': test_metrics.get('auc', 0) if test_metrics.get('auc') is not None else 0,
                    'Top-3 Acc': test_metrics.get('top_3_accuracy', 0) if test_metrics.get('top_3_accuracy') is not None else 0
                })
    ...
    for model_name in ml_models:
        ...
        with tab5:
            ...
            st.info("Training history plots are available for Deep Learning and Transformer models only.")
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #666; padding: 2rem;'>
            ...
            <p>2 ML Models | 2 DL Models | 1 Transformer Model</p>
        </div>
        """,
        unsafe_allow_html=True
    )
